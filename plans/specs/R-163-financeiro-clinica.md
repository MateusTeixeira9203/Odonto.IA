...
## 2. Decisões propostas e confirmação necessária

- `responsavelClinico`, `atorFinanceiro` e `titularFinanceiro` são conceitos distintos.
- Cada orçamento tem um titular financeiro, imutável após aceite/cobrança/recebimento.
- Configuração da unidade: recebe a clínica, recebem os dentistas ou ambos. No modo misto,
  escolher titular antes de emitir proposta; não deduzir a partir de quem está logado.
- Primeira entrega preserva pagamento direto e caixa centralizado. Repasses são informativos
  ou lançamentos manuais vinculados; percentuais, aluguel e liquidação automática ficam posteriores.
- Proposta por tabela aparece imediatamente na lista; aceite e pagamento são fatos separados.

**Ainda precisa de aceite do usuário:** os três modos de titularidade e qual será padrão no
piloto. Nada no histórico autoriza migrar recebimentos existentes para o dono da clínica.
