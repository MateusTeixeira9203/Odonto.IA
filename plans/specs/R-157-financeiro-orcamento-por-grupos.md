...
Caso de referência: Renato, R$ 30.000; arcada superior R$ 15.000 e inferior R$ 15.000.
A cliente Leia voltou a acessar segundo relato do usuário; isso não encerra R-155.
...
### Apêndice de integração — guarda SQL de acordo global

O modal não é a fronteira de segurança: além das RPCs R-157/R-169, SQL direto e as RPCs
legadas de plano não podem criar `valor_acordado`, `plano_forma`, `desconto` ou pagamento
global (`cobranca_id is null`) quando existe grupo ativo, inclusive ao desvincular uma etapa. A migration aditiva
`20260915183139_r157_bloquear_plano_global_grupos.sql` serializa esses caminhos pela linha do
orçamento e retorna `grupo_orcamento_negociado`; cobrança por etapa continua permitida.

O filtro considera somente grupos não retirados (`retirado_em is null`), preservando R-169.
Não retroconverte orçamento histórico já misto: deixa metadados e a remoção dos campos globais
prosseguirem. Aceite: `supabase/tests/r157_guard_plano_global_grupos.sql`, transação sintética
com `ROLLBACK`, cobre acordo/plano/desconto/pagamento global bloqueados, inclusão de grupo em
orçamento já negociado, preservação do legado e grupo retirado.
