      ...
      <main className="px-4 pt-4 pb-8 sm:px-6 sm:pt-6 lg:ml-[248px] lg:px-12 lg:pt-8 lg:pb-14">
        <header className="mb-8 flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex w-full flex-wrap items-center justify-end gap-2 sm:w-auto sm:flex-nowrap">
            ...
            {canReadStock ? <Link href="/estoque" className="inline-flex min-h-11 items-center rounded-lg border border-border bg-card px-3 text-sm font-semibold text-foreground transition-colors hover:bg-surface-alt focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring motion-reduce:transition-none">Estoque</Link> : null}
...
export function TeamWorkspace({ initialResult, canAttend, canReadStock, loadPage }: TeamWorkspaceProps) {
  ...
  if (!page) {
    return (
      <TeamScreenFrame canAttend={false} canReadStock={canReadStock} isLoggingOut={isLoggingOut} onLogout={() => { void logout(); }}>
  ...
  return (
    <TeamScreenFrame clinicaNome={page.clinicaNome} canAttend={canAttend} canReadStock={canReadStock} isLoggingOut={isLoggingOut} onLogout={() => { void logout(); }}>
