...
export async function calcularSaldoMes(mesISO: string, dentistaFiltro?: string): Promise<SaldoMes> {
  ...
  if (isTeamWorkspaceEnabled() && context.role !== 'secretaria') pagamentosQuery = pagamentosQuery.eq('titular_recebimento', 'dentista');
...
export async function exportarFinanceiroCsv(
  ...
  if (isTeamWorkspaceEnabled() && context.role !== 'secretaria') pagamentosQ = pagamentosQ.eq('titular_recebimento', 'dentista');
...
export async function listarPagamentosPagos(mesISO: string, dentistaFiltro?: string): Promise<PagamentoPago[]> {
  ...
  if (isTeamWorkspaceEnabled() && context.role !== 'secretaria') query = query.eq('titular_recebimento', 'dentista');
...
export async function buscarOrcamentosPendentesPorPaciente(
  ...
  let cobrancasQuery = supabase
    ...
    .eq('situacao', 'aberta');
  if (isTeamWorkspaceEnabled() && context.role !== 'secretaria') cobrancasQuery = cobrancasQuery.eq('titular_recebimento', 'dentista');
  ...
  let orcamentosQuery = supabase
    .from('orcamentos')
    .select('id, total, valor_acordado, desconto, dentista_id, itens:orcamento_itens(descricao, preco_total, aprovado), cobrancas:orcamento_cobrancas(desconto, situacao), pagamentos(id, valor, status)')
    .eq('clinica_id', clinicId)
    .eq('paciente_id', paciente.data)
    .is('itens.retirado_em', null);
  if (isTeamWorkspaceEnabled() && context.role !== 'secretaria') orcamentosQuery = orcamentosQuery.eq('titular_recebimento', 'dentista');
