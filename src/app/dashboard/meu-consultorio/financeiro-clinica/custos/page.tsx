...
export default async function ClinicFixedCostsPage(): Promise<React.JSX.Element> {
  const context = await getClinicHubContext();
  if (!context.ok) redirect('/onboarding');
  if (!context.data.member.perfilClinico) redirect('/consultorio/financeiro-clinica/custos');
  const financeiro = await getClinicFinancial({ clinicaIdEsperada: context.data.member.clinicaId, mes: format(new Date(), 'yyyy-MM') });
  return <FixedCostsPage basePath={context.data.basePath} data={financeiro.ok ? financeiro.data : null} mensagem={financeiro.ok ? undefined : financeiro.mensagem} />;
}
