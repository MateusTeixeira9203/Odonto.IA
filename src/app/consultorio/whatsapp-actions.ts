...
export async function saveOperationalWhatsAppTemplates(_: WhatsAppTemplateState, formData: FormData): Promise<WhatsAppTemplateState> {
  ...
  const parsed = z.object({ confirmacao: TemplateSchema, lembrete_24h: TemplateSchema, reativacao: TemplateSchema, cobranca: TemplateSchema, orcamento: TemplateSchema }).safeParse({
    confirmacao: formData.get('confirmacao'), lembrete_24h: formData.get('lembrete_24h'), reativacao: formData.get('reativacao'), cobranca: formData.get('cobranca'), orcamento: formData.get('orcamento'),
  });
