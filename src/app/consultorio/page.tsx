...
export default async function ConsultorioPage(): Promise<React.JSX.Element> {
  ...
  const financeiro = await getClinicFinancial({
    clinicaIdEsperada: context.data.member.clinicaId,
    mes: format(new Date(), 'yyyy-MM'),
  });

  return <PageTransition><PageContainer variant="wide"><ClinicOverviewPanel
    basePath={context.data.basePath}
    nomeClinica={context.data.nomeClinica}
    title={context.data.titulo}
    hasPersonalFinance={false}
    financeiro={financeiro.ok ? financeiro.data : null}
