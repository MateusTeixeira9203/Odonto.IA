  ...
  return (
    <div className="space-y-10">
      ...
      <section className="grid gap-3 lg:grid-cols-3" aria-label="Pendências da clínica">
        <AttentionCard icon={FileClock} label="Orçamentos sem retorno" value={String(overview.orcamentosSemRetorno.quantidade)} detail={money.format(overview.orcamentosSemRetorno.valor) + ' aguardando decisão'} href="/dashboard/orcamentos" />
        <AttentionCard icon={UserRoundCheck} label="Pacientes para reativar" value={String(overview.pacientesParaReativar)} detail="Follow-ups pendentes da equipe" href="/dashboard/pacientes" />
        <AttentionCard icon={CalendarClock} label="Pagamentos vencidos" value={String(overview.pagamentosVencidos.quantidade)} detail={money.format(overview.pagamentosVencidos.valor) + ' em atraso'} href={basePath + '/financeiro-clinica'} />
      </section>
...
function AttentionCard({ icon: Icon, label, value, detail, href }: { icon: typeof CircleDollarSign; label: string; value: string; detail: string; href: string }): React.JSX.Element {
  return <Link href={href} className="group rounded-2xl border border-border bg-card p-5 transition-colors hover:border-teal/40"><div className="flex items-center justify-between"><Icon className="size-5 text-amber" /><ArrowRight className="size-4 text-muted-foreground transition-transform group-hover:translate-x-0.5" /></div><p className="mt-5 text-sm font-semibold text-foreground">{label}</p><p className="mt-2 font-mono text-3xl font-semibold text-foreground">{value}</p><p className="mt-1 text-xs text-muted-foreground">{detail}</p></Link>;
}
