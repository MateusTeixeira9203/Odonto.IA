...
import { createPersonalTransaction, type PersonalTransactionState } from '@/app/dashboard/meu-consultorio/meu-financeiro/actions';
import { Button } from '@/components/ui/button';
import { Dialog, DialogContent, DialogDescription, DialogHeader, DialogTitle } from '@/components/ui/dialog';
import { Input } from '@/components/ui/input';

const initialState: PersonalTransactionState = { ok: false, message: '' };

export function PersonalTransactionActions(): React.JSX.Element {
  const [type, setType] = useState<'entrada' | 'saida' | null>(null);
  const [state, action, pending] = useActionState(createPersonalTransaction, initialState);

  return <>
    <div className="flex flex-wrap gap-2">
      <Button variant="outline" className="min-h-11" onClick={() => setType('saida')}><ArrowUpRight className="size-4" />Registrar custo</Button>
    ...
    <Dialog open={type !== null} onOpenChange={(open) => !open && setType(null)}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{type === 'saida' ? 'Registrar custo profissional' : 'Registrar entrada pessoal'}</DialogTitle>
          <DialogDescription>Este lançamento pertence ao seu financeiro. Recebimentos de pacientes ficam no caixa da clínica.</DialogDescription>
        </DialogHeader>
        {type && <form action={action} className="space-y-4">
          ...
          {type === 'saida'
            ? <label className="block text-sm font-medium text-foreground">Categoria<Input className="mt-2" name="categoria" placeholder="Ex.: material próprio, curso" required /></label>
          ...
          <Button type="submit" className="min-h-11 w-full" disabled={pending}>{pending ? 'Registrando…' : 'Confirmar lançamento'}</Button>
        </form>}
      </DialogContent>
    </Dialog>
  </>;
