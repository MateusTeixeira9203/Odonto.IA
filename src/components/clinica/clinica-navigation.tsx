...
export function ClinicaNavigation({ podeAtender, podeReceber, nome }: { podeAtender: boolean; podeReceber: boolean; nome: string }) {
  const pathname = usePathname();
  const tabs = [{ href: '/clinica', label: 'Resultados da clínica' }, ...(podeReceber ? [{ href: '/dashboard/recebimentos', label: 'Recebimentos' }] : []), ...(podeAtender ? [{ href: '/clinica/meus-resultados', label: 'Meus resultados' }, { href: '/dashboard/meu-consultorio/precos', label: 'Procedimentos' }, { href: '/dashboard/meu-consultorio/estoque', label: 'Estoque' }] : [])];
