...
test('novas ações de estoque aceitam os escopos previstos', () => {
  const novasAcoes = ['estoque.receber', 'estoque.consumir', 'estoque.descartar'] as const;
  const escopos = [
    scope('nenhum'),
    scope('proprio'),
    scope('selecionados', [dentistA]),
    scope('clinica'),
  ];

  novasAcoes.forEach((permissao) => {
    escopos.forEach((escopo) => {
      assert.equal(AccessSchema.safeParse({ permissao, escopo }).success, true);
    });
  });
});
...
test('coleção completa de 45 permissões continua válida', () => {
  ...
  assert.equal(AccessCollectionSchema.safeParse(collection).success, true);
});
