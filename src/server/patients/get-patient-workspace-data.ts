...
export type CobrancaEtapa = {
  ...
  parcelas_forma?: 'acordo' | 'cartao_credito';
  id: string;
  subtotal: number;
  desconto: number;
  valor_final: number;
...
export async function getPatientWorkspaceData({
  ...
  if (orcamentoIds.length > 0) {
    const { data, error } = await supabase
      ...
      .select(
        'id, orcamento_id, observacoes, entrada_valor, entrada_forma, entrada_registrada, parcelas_forma, subtotal, desconto, valor_final, numero_parcelas, primeiro_vencimento, situacao, created_at, itens:orcamento_cobranca_itens!orcamento_cobranca_itens_cobranca_id_fkey(orcamento_item_id, preco_total_snapshot), pagamentos:pagamentos!pagamentos_cobranca_id_fkey(id, cobranca_id, valor, status, forma_pagamento, data_pagamento, data_vencimento, parcela_numero, total_parcelas, marcado_por:dentistas!pagamentos_marcado_por_id_fkey(nome))'
      )
      .eq('clinica_id', clinicId)
