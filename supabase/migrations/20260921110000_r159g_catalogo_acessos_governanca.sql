-- R-159g: sincroniza o catálogo SQL com as 48 capacidades validadas no servidor.
-- Tabelas de acesso estão vazias no principal; não há conversão de dados.
...
begin
  if p_acessos is null
    ...
    or jsonb_array_length(p_acessos) > 48 then
    return null;
  end if;
  ...
  loop
    ...
    if v_permissao = any(v_permissoes)
      or v_permissao not in (
        ...
        'contatos.whatsapp', 'acompanhamentos.ler', 'acompanhamentos.gerir',
        'clinico.ler', 'clinico.registrar', 'clinico.revisar_tecnicamente',
        'prontuario.exportar',
        'orcamentos.ler', 'orcamentos.criar', 'orcamentos.aceite', 'orcamentos.cancelar',
        'precos.ler', 'precos.editar', 'precos.excecao',
        'descontos.solicitar', 'descontos.aprovar',
        'cobrancas.ler', 'cobrancas.gerir',
        ...
        'governanca.gerir', 'configuracoes.gerir', 'auditoria.ler',
        'estoque.ler', 'estoque.gerir', 'estoque.receber', 'estoque.consumir',
        'estoque.descartar', 'estoque.ajustar', 'kits.gerir', 'materiais.confirmar',
        'unidades.consolidar'
      ) then
      return null;
    end if;
    ...
    if (v_permissao in ('pacientes.ler', 'pacientes.editar')
      ...
      or (v_permissao in (
            'clinico.revisar_tecnicamente', 'prontuario.exportar',
            'precos.ler', 'precos.editar', 'equipe.ler', 'equipe.convidar',
            'equipe.remover', 'permissoes.gerir', 'governanca.gerir',
            'configuracoes.gerir', 'auditoria.ler', 'unidades.consolidar'
...
revoke all on function private.normalizar_acessos(jsonb) from public, anon, authenticated, service_role;
