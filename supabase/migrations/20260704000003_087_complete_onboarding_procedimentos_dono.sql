-- 087 — Corrige complete_onboarding: a cópia dos procedimentos_padrao (passo 9)
-- não definia dentista_id. Depois da migration 084 (catálogo privado por
-- dentista), essas linhas ficavam com dentista_id NULL — invisíveis pro
-- próprio dentista fundador que acabou de se cadastrar (só admin/secretaria
-- enxergam NULL). Mesma assinatura da 086, só o corpo muda (passo 9).
--
-- Nota: não há backfill de dados aqui. Os únicos registros com dentista_id
-- NULL em produção pertencem a uma clínica sem nenhum dentista/membership
-- (dado morto de teste, anterior a esta leva de migrations) — não são
-- consequência deste bug e não há dono real pra atribuir.
...
BEGIN
  ...
  -- ── 9. Copiar procedimentos padrão pro dentista recém-criado (best-effort) ───
  -- Fix 087: dentista_id precisa ser setado — a partir da 084 o catálogo é
  -- privado por dentista, sem isso a linha nasce órfã (invisível pro dono).
  BEGIN
    INSERT INTO procedimentos (
      clinica_id, dentista_id, nome, descricao, categoria,
