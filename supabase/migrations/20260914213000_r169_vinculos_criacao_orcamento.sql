...
begin
  ...
  -- Serialize com retirada clínica, mantendo todos os IDs em ordem estável.
    ...
    loop
      if not exists (
        ...
        where e.id = v_evento_id
          ...
          and e.status in ('indicado', 'realizado')
          and e.paciente_id = v_orcamento.paciente_id
...
begin
    ...
    loop
      if not exists (
        ...
        where e.id = v_evento_id
          ...
          and (p_ficha_id is null or e.ficha_id = p_ficha_id)
          and f.paciente_id = p_paciente_id
          and e.origem = 'clinica'
  ...
  loop
    ...
    loop
      insert into public.orcamento_eventos (clinica_id, orcamento_id, evento_id, item_id)
      values (v_clinica_id, v_orcamento_id, v_evento_id, v_item_id);
    end loop;
  end loop;
