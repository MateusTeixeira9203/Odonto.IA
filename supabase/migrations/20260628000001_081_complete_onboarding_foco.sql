-- Migration 081: complete_onboarding aceita p_foco_principal (persona).
--
-- Fase 1 retenção (Workstream A/E): a persona escolhida no onboarding
-- ('economizar_tempo' | 'crescer') é gravada atomicamente na criação do
-- dentista, dentro da mesma transação da RPC — sem segundo round-trip nem
-- dependência de policy RLS.
--
-- Compatibilidade: p_foco_principal entra como ÚLTIMO parâmetro, DEFAULT NULL.
-- Dropamos a versão de 9 args e recriamos com 10. Chamadas antigas (9 args, da
-- app ainda não deployada) resolvem para a nova função com foco = NULL — nada
-- quebra em produção até o deploy do código novo.
...
-- ── Permissões (assinatura de 10 args) ───────────────────────────────────────────
