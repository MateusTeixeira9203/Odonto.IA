-- R163a: compatibilidade com recebimentos legados sem profissional/etapa.
...
begin
  ...
  with profissionais as (
    ...
    select c.dentista_id,c.titular_recebimento,greatest(0,
      sum(greatest(0,round(c.valor_final*100)-coalesce((select sum(round(p.valor*100)) from public.pagamentos p
        where p.clinica_id=c.clinica_id and p.cobranca_id=c.id and p.status='pago'),0)))
      - coalesce((select sum(round(p.valor*100)) from public.pagamentos p
        where p.clinica_id=c.clinica_id and p.orcamento_id=c.orcamento_id and p.cobranca_id is null and p.status='pago'),0)) as valor
    from public.orcamento_cobrancas c where c.clinica_id=p_clinica_id_esperada and c.situacao='aberta'
    group by c.clinica_id,c.orcamento_id,c.dentista_id,c.titular_recebimento
    union all
