-- R-172 — revisão comercial de orçamento já aceito.
--
-- A edição legado apaga e recria todos os itens; isso perde a semântica da aprovação
-- por item. Esta RPC atualiza somente os itens que já existem, mantém seus IDs/aprovações
-- e cria itens novos como aceitos porque a UI exige a confirmação explícita do dentista.
-- Pagamentos pagos/cancelados nunca são tocados; previsões pendentes são redistribuídas
-- pelo mesmo contrato de R-145.
...
begin
  ...
  -- R-172 não remove item: todo item existente precisa reaparecer uma vez no payload.
