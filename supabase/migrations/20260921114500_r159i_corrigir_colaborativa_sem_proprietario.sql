-- R-159i: corrige a configuração inicial para colaborativa sem proprietário.
-- Substitui a RPC R-159h antes de ela ter qualquer consumidor.
...
begin
  ...
  if (p_modalidade = 'gerida' and p_proprietario_membro_id is null)
    or (p_modalidade = 'colaborativa' and p_proprietario_membro_id is not null) then
    return jsonb_build_object('ok', false, 'codigo', 'INVALIDO', 'mensagem', 'A modalidade e a propriedade inicial são incompatíveis.');
  end if;

  -- Em clínica gerida, o próprio criador aceita a primeira propriedade.
  if p_modalidade = 'gerida' and p_proprietario_membro_id <> v_ator_membro_id then
  ...
  v_depois := jsonb_build_object(
    ...
    'resultado', jsonb_build_object(
      ...
      'proprietarioMembroId', case when p_modalidade = 'gerida' then v_ator_membro_id::text else null end
    )
  );
  ...
  insert into public.clinica_governanca_auditoria (
    ...
    jsonb_build_object('modalidade', null, 'proprietarioMembroId', null, 'versao', 0),
    v_depois,
