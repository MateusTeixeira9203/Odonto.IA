begin;
-- Após a primeira retirada, manter estas guardas mesmo revertendo a aplicação.
do $$ begin
 if exists(select 1 from public.odontograma_eventos e where (to_jsonb(e)->>'retirado_em') is not null) then
 raise exception 'ha_historico_retirado_preserve_guardas'; end if;
end $$;
